...
bool isValid(const int row, const int column) {
  return (row >= 0) && (row < static_cast<int>(TileArrayHeight)) &&
         (column >= 0) && (column < static_cast<int>(TileArrayWidth));
}

// check that cell is free
bool isFreeCell(const Tile (&grid)[TileArrayHeight][TileArrayWidth],
                const int row, const int column) {
  return grid[row][column] == Tile::Grass ||
         grid[row][column] == Tile::EmptyGrass;
}

// check destination
bool isFinished(const int row, const int column,
                const std::pair<int, int> &dest) {
  return row == dest.first && column == dest.second;
}

// calculate heuristic
double calculateHeuristic(const int row, const int col,
                          const std::pair<int, int> &dest) {
  // distance
...
std::pair<int, int> getFirstCell(Cell cellDetails[][TileArrayWidth],
  ...
  while (!(cellDetails[row][column].parentRow == row &&
...
std::pair<int, int> findBestCell(Tile (&grid)[TileArrayHeight][TileArrayWidth],
  ...
  if (!isFreeCell(grid, src.first, src.second) ||
      !isFreeCell(grid, dest.first, dest.second)) {
    return ret_val_error;
  }
  ...
  while (!openList.empty()) {
    ...
    for (int dirIter = 0; dirIter < directions; dirIter++) {
      if (isValid(i + cellDir[dirIter][0], j + cellDir[dirIter][1])) {
        // check if finished
        if (isFinished(i + cellDir[dirIter][0], j + cellDir[dirIter][1],
                       dest)) {
          // Set the Parent of the destination cell
          cellDetails[i + cellDir[dirIter][0]][j + cellDir[dirIter][1]]
              .parentRow = i;
          cellDetails[i + cellDir[dirIter][0]][j + cellDir[dirIter][1]]
              .parentColumn = j;
        ...
        else if (!closedList[i + cellDir[dirIter][0]]
                            [j + cellDir[dirIter][1]] &&
                 isFreeCell(grid, i + cellDir[dirIter][0],
                            j + cellDir[dirIter][1])) {
          // new values
          gNew = cellDetails[i][j].g + 1.0;
          hNew = calculateHeuristic(i + cellDir[dirIter][0],
                                    j + cellDir[dirIter][1], dest);
          fNew = gNew + hNew;

          if (cellDetails[i + cellDir[dirIter][0]][j + cellDir[dirIter][1]].f ==
                  FLT_MAX ||
              cellDetails[i + cellDir[dirIter][0]][j + cellDir[dirIter][1]].f >
                  fNew) {
            openList.insert(
                std::make_pair(fNew, std::make_pair(i + cellDir[dirIter][0],
                                                    j + cellDir[dirIter][1])));

            // Update the details of this cell
            cellDetails[i + cellDir[dirIter][0]][j + cellDir[dirIter][1]].f =
                fNew;
            cellDetails[i + cellDir[dirIter][0]][j + cellDir[dirIter][1]].g =
                gNew;
            cellDetails[i + cellDir[dirIter][0]][j + cellDir[dirIter][1]].h =
                hNew;
            cellDetails[i + cellDir[dirIter][0]][j + cellDir[dirIter][1]]
                .parentRow = i;
            cellDetails[i + cellDir[dirIter][0]][j + cellDir[dirIter][1]]
